    ...
    const fetchPosts = async () => {
      try {
        const response = await fetch(
          "https://awt-backend.onrender.com/api/awt/blogs/Get"
        );
        if (!response.ok) {
    ...
    <div className="flex justify-start flex-col items-start gap-4 w-11/12 py-12 mx-auto">
      ...
      <div className="w-full flex justify-center items-center py-4">
        <Pagination
          ...
          classNames={{
            wrapper:
              "gap-0 overflow-visible h-10 p-1 rounded border border-divider",
            item: "w-8 h-8 text-small rounded-none bg-transparent",
            cursor: " bg-gradient-to-b shadow-lg from-[#FF7143] to-[#FF7143]",
          }}
        />
      </div>
    </div>
