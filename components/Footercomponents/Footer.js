    ...
    <div className="w-full flex flex-col bg-black">
      ...
      <div className="bg-black md:h-40 lg:h-40 h-56 md:pb-0 lg:pb-0 pb-4  w-full gap-8 flex flex-col md:flex-row lg:flex-row justify-between md:items-center lg:items-center items-start md:px-12 lg:px-12 px-4 ">
           <div className="flex justify-start items-start gap-2 md:w-1/2 lg:w-1/2 w-full flex-col">
            <h6 className="text-xs font-medium text-[#FF7143] uppercase">newsletter</h6>
            <p className="text-[0.7rem] md:text-xs lg:text-xs font-medium text-white md:leading-7 lg:leading-7 leading-7 uppercase">{`Sign up for our newsletter now! Get exclusive insights, trends, and strategies to fuel your business growth. Stay informed, stay ahead. Subscribe today!`}</p>
           </div>
           <div className="flex items-center h-12 ring-1 ring-white rounded-lg md:w-1/2 lg:w-1/2 w-full px-4">
            <input placeholder="Enter Your Email" className="border-none text-white w-11/12 focus:outline-none bg-black p-2"/>
            <span className="text-sm font-extrabold text-[#FF7143] uppercase">send</span>
           </div>
      </div>
        ...
        <div className="w-11/12 mx-auto flex md:flex-row flex-col justify-between items-start md:px-12">
          <div className="flex flex-wrap md:gap-24 lg:gap-24 gap-4 w-full items-start">
            <div className="flex flex-col gap-4 justify-start items-start text-white uppercase">
              <p className="text-[#FF7143] font-semibold md:text-sm lg:text-sm text-[0.7rem]">company</p>
              <span onClick={()=>router.push('/Aboutus')} className="md:text-xs hover:text-[#FF7143] lg:text-xs text-[0.7rem] text-white cursor-pointer">
                About Us
              </span>
              <span onClick={()=>router.push('/Oursolution')} className="md:text-xs hover:text-[#FF7143] lg:text-xs text-[0.7rem] text-white cursor-pointer">
              Our Solutions
              </span>
              <span onClick={()=>router.push('/Products')} className="md:text-xs hover:text-[#FF7143] lg:text-xs text-[0.7rem] text-white cursor-pointer">
              our projects
              </span>
              <span onClick={()=>router.push('/Products')} className="md:text-xs hover:text-[#FF7143] lg:text-xs text-[0.7rem] text-white cursor-pointer">
              products
              </span>
              <span onClick={()=>router.push('/Blog')} className="md:text-xs hover:text-[#FF7143] lg:text-xs text-[0.7rem] text-white cursor-pointer">
              blog
              </span>
            </div>
            <div className="flex flex-col gap-4 justify-start items-start text-white uppercase">
              <p className="text-[#FF7143] font-semibold md:text-xs lg:text-xs text-[0.7rem]">solutions</p>
              <span onClick={()=>router.push('/Oursolution')} className="md:text-xs hover:text-[#FF7143] lg:text-xs text-[0.7rem] text-white cursor-pointer">
              ui ux design
              </span>
              <span onClick={()=>router.push('/Oursolution')} className="md:text-xs hover:text-[#FF7143] lg:text-xs text-[0.7rem] text-white cursor-pointer">
            ...
            <div className="flex flex-col gap-4 justify-start items-start text-white uppercase">
              <p className="text-[#FF7143] font-semibold md:text-xs lg:text-xs text-[0.7rem]">legal</p>
              <span onClick={()=>router.push('/Termsandconditions')} className="md:text-xs lg:text-xs text-[0.6rem] text-white cursor-pointer hover:text-[#FF7143]">
              terms & conditions
              </span>
              <span onClick={()=>router.push('/Policy')}  className="md:text-xs lg:text-xs text-[0.7rem] text-white cursor-pointer hover:text-[#FF7143]">
              privacy policy
              </span>
            </div>
          </div>
